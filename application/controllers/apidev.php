...
*/
	...
	public function getMessageiOS()
	{
		...
		{	
			$data['json']->id = "201210181040";
			$data['json']->title = "*ประกาศ*";
			$data['json']->message = "เนื่องจาก Server ใหม่มีปัญหาทำให้ต้องย้ายกลับมา Server เดิมครับ";
...
/* 			array_push($buttons, $this->Tv_model->createButton('Read','http://goo.gl/NeDgB')); */
	...
	{
		$cache_key = $this->namespace_prefix.'getProgram_'.$cat_id.'_'.$start;
		$memData = $this->memcached->get($cache_key);
		if(FALSE != $memData)
		{
			echo $memData;
...
/* 		$this->load->view('json',$data); */
	...
	{
		$cache_key = $this->namespace_prefix.'getWhatsNew_'.$start;
		$memData = $this->memcached->get($cache_key);
		if(FALSE != $memData)
		{
			echo $memData;
...
/* 		$this->load->view('json',$data); */
	...
	public function getProgramSearch($start = 0)
	{
		...
		$cache_key = $this->namespace_prefix.'getProgramSearch_'.$keyword.'_'.$start;
		$memData = $this->memcached->get($cache_key);
		if(FALSE != $memData)
		{
			echo $memData;
...
*/
	...
	public function clearCache($program_id = 0)
	{
		if($program_id == 0)
		{
			...
			{
				$this->memcached->delete($this->namespace_prefix.'getProgram_0_'.$start);
				$this->memcached->delete($this->namespace_prefix.'getProgramlist_'.$program_id.'_'.$start);
			}
			...
			if($program != FALSE)
			{
				...
				for($start = 0;$start <= 400;$start = $start + 20)
				{
					$this->memcached->delete($this->namespace_prefix.'getProgram_'.$program->category_id.'_'.$start);
					$this->memcached->delete($this->namespace_prefix.'getProgram_'.$program->channel_id.'_'.$start);
				}
			}
			else
			{
