...
class Api4 extends CI_Controller {
	...
	public function message() {
		...
		if(FALSE == $memData)
		{
			$this->load->model('Tv4_model','', TRUE);
			$this->Tv4_model->setDevice($this->device);
			$result = $this->Tv4_model->getMessage();
			...
			if($this->device == 'android') {
				array_push($buttons, $this->Tv4_model->createButton('Rating & Review','http://goo.gl/1BJYa'));
				array_push($buttons, $this->Tv4_model->createButton('Fan Page','https://www.facebook.com/TV.Thailand'));
			}
			elseif($this->device == 'ios') {
				array_push($buttons, $this->Tv4_model->createButton('Rating & Review','https://itunes.apple.com/us/app/tv-thailand/id458429827?ls=1&mt=8'));
				array_push($buttons, $this->Tv4_model->createButton('Fan Page','https://www.facebook.com/TV.Thailand'));
			}
	...
	public function advertise()
	{
		...
		if(FALSE == $memData)
		{
			...
			$data->ads = $this->Tv4_model->getAdvertise();
	...
	public function preroll_advertise()
	{
		...
		if(FALSE == $memData)
		{
			...
			$this->Tv4_model->setDevice($this->device);
			$data = new stdClass();
			$data->ads = $this->Tv4_model->getPrerollAdvertise();
			$memData = json_encode($data);
			$this->cache->redis->save($cache_key, $memData, $this->cache_time);
		}
		$this->output->set_content_type('application/json')->set_output($memData);
	...
	public function section() {
		...
		if(FALSE == $memData)
		{
			$this->load->model('Tv4_model','', TRUE);
			$this->Tv4_model->setDevice($this->device);
			$this->Tv4_model->setIsTH($this->isTH);

			$data = new stdClass();
			$data->categories = $this->Tv4_model->getCategory();
			$data->channels = $this->Tv4_model->getChannel();
			$data->radios = $this->Tv4_model->getRadio();
	...
	public function all_program()
	{
		...
		if(FALSE == $memData)
		{
			...
			$data = new stdClass();
			$data->programs = $this->Tv4_model->getAllProgram();
			$memData = json_encode($data);
			$this->cache->redis->save($cache_key, $memData, $this->cache_time);
		}
		$this->output->set_content_type('application/json')->set_output($memData);
	...
	private function _getProgramRecents($start = 0) {
		...
		$data = new stdClass();
		$data->programs = $this->Tv4_model->getWhatsNewProgram($start);
	...
	private function _getProgramTopHits($start = 0) {
		...
		$data = new stdClass();
		$data->programs = $this->Tv4_model->getProgramByTopHits($start);
	...
	private function _getProgramByCategory($id, $start = 0) {
		...
		$data = new stdClass();
		$data->programs = $this->Tv4_model->getProgramByCategory($id, $start);
		return json_encode($data);
	}
	...
	private function _getProgramByChannel($id, $start = 0) {
		...
		$data = new stdClass();
		$data->programs = $this->Tv4_model->getProgramByChannel($id, $start);
		return json_encode($data);
	}
	...
	private function _getProgramBySearch($keyword, $start = 0) {
		...
		$data = new stdClass();
		$data->programs = $this->Tv4_model->getProgramSearch($keyword, $start);
		return json_encode($data);
	}
	...
	public function episode($id, $start = 0) {
		...
		if(FALSE == $memData)
		{
			...
			$data->code = 200;
			if($start == 0) {
				$data->info = $this->Tv4_model->getProgramInfo($id);
			}
			$data->episodes = $this->Tv4_model->getEpisode($id, $start);
			$memData = json_encode($data);
			$this->cache->redis->save($cache_key, $memData, $this->cache_time);
	...
	public function episode_raw($id, $start = 0) {
		...
		if(FALSE == $memData)
		{
			...
			$data->episodes = $this->Tv4_model->getEpisodeRaw($id, $start);
			$memData = json_encode($data);
			$this->cache->redis->save($cache_key, $memData, $this->cache_time);
			$this->storeKey($this->getProgramKey($id), $cache_key);
		}
	...
	public function program_info($id)
	{
		...
		if(FALSE == $memData)
		{
			$this->load->model('Tv4_model','', TRUE);
			$data = new stdClass();
			$data = $this->Tv4_model->getProgramInfo($id);
			$memData = json_encode($data);
			$this->cache->redis->save($cache_key, $memData, $this->cache_time);
		}
		$this->output->set_content_type('application/json')->set_output($memData);
	...
	public function program_info_otv($id)
	{
		...
		if(FALSE == $memData)
		{
			$this->load->model('Tv4_model','', TRUE);
			$data = new stdClass();
			$data = $this->Tv4_model->getProgramInfoOtv($id);
			$memData = json_encode($data);
			$this->cache->redis->save($cache_key, $memData, $this->cache_time);
		}
		$this->output->set_content_type('application/json')->set_output($memData);
