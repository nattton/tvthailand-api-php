...
class Tv4_model extends CI_Model
{
	...
	function getProgramByCategory($id, $start = 0) {
		...
		CASE otv_logo WHEN '' THEN '' ELSE CONCAT('$this->otv_logo_path', otv_logo) END AS otv_logo  
		FROM tv_program 
		WHERE online = 1 AND is_otv = 1 AND category_id = $id";
	...
	function getProgramByChannel($id, $start = 0) {
		...
		CASE otv_logo WHEN '' THEN '' ELSE CONCAT('$this->otv_logo_path', otv_logo) END AS otv_logo  
		FROM tv_program 
		WHERE online = 1 AND is_otv = 1 AND channel_id = $id";
	...
	function getProgramSearch($keyword = '',$start = 0)
	{
		...
		$special = 'tv_program.online = 1 AND tv_program.is_otv = 1 AND';
		if(strpos($keyword, '@') === 0) {
			$special = '';
